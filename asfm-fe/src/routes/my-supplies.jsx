...
function RouteComponent() {
  ...
  useEffect(() => {
    try {
      if (mockUser.role !== 'USER') {
        ...
        const filtered = mockLoanedItems.filter(
          (item) => item.userId === mockUser.userId
        )
      ...
      setError('Failed to load my supplies. Please try again.')
      setLoading(false)
    }
  }, [])
  ...
  return (
    <Layout navBar={<BasicNavBar />}>
      <div className="flex justify-center pt-2">My Supplies</div>
